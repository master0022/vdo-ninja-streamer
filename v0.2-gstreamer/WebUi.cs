...
.grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 11px; }.field { min-width: 0; }.field label { display: block; color: var(--muted); font-size: 11px; margin: 0 0 6px; }.field.wide { grid-column: span 2; }
.presets { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }.preset { text-align: left; min-height: 55px; }.preset strong { display: block; font-size: 12px; }.preset small { display: block; color: var(--muted); margin-top: 3px; font-size: 10px; }
...
<main class="shell">
  ...
  <div class="layout">
    <section class="stack">
      <article class="card">
        <div class="source-head"><div><h2>Video source</h2><p class="sub">Capture one application window or an entire monitor.</p></div><button id="refresh" class="btn ghost">↻ Refresh</button></div>
        <div class="grid">
          <div class="field"><label>Capture mode</label><select id="videoSource"><option value="Window">Application window</option><option value="Monitor">Entire monitor</option></select></div>
          <div id="windowSource" class="field wide"><label>Window</label><select id="window"></select></div>
          <div id="monitorSource" class="field wide" hidden><label>Monitor</label><select id="monitorIndex"></select></div>
        </div>
        <div class="window-meta"><span id="windowTitle">No video source selected</span><span id="windowPid" class="pill">—</span></div>
        <div class="field" style="margin-top:12px"><label>Audio source</label><select id="audioSource"><option value="SelectedProcess">Selected app audio</option><option value="SystemExceptDiscord">Entire PC audio (except Discord)</option></select><p id="audioNote" class="hint">Captures the selected window's process tree.</p></div>
      </article>
      <article class="card">
        <h2>Stream link</h2><p class="sub">Your stream key is also the public link slug — it is intentionally visible.</p>
        <div class="key-row"><input id="streamKey" type="text" placeholder="e.g. thales-main" autocomplete="off"><button id="copyKey" class="btn">Copy key</button></div>
        <div class="link-box"><a id="viewerLink" class="link empty" href="#">Enter a stream key to create the viewer link</a><button id="copyLink" class="btn">Copy link</button></div>
        <p class="hint">Share this link with viewers. The key is sent as the WHIP bearer token.</p>
      </article>
      <article class="card">
        <h2>Video and audio</h2><p class="sub">Keep the budget preset for older PCs; use Lanczos when downscaling matters more than CPU use.</p>
        <div class="grid">
          <div class="field"><label>Encoder</label><select id="encoder"></select><p id="encoderNote" class="hint"></p></div>
          ...
          <div class="field"><label>Encoder preset</label><input id="encoderPreset" type="text" placeholder="p1 / ultrafast"></div>
          ...
          <div class="field"><label>Audio gain (x)</label><input id="audioGain" type="number" min="0.1" max="4" step="0.1"></div>
          ...
          <div class="field"><label>CRF / quality</label><input id="crf" type="number" min="0" max="51"></div>
          <div class="field wide"><label>WHIP endpoint</label><input id="endpoint" type="text"></div>
        </div>
      </article>
    </section>
    <aside class="stack">
      <article class="card"><h2>Quick presets</h2><p class="sub">Presets fill the fields. You can still edit everything.</p>
        <div class="presets"><button class="btn preset" data-preset="budget"><strong>Budget PC</strong><small>720p · 30 FPS · NVENC</small></button><button class="btn preset" data-preset="motion"><strong>Motion 60</strong><small>720p · 60 FPS · 3.5 Mbps</small></button><button class="btn preset" data-preset="quality"><strong>4K → 720</strong><small>Lanczos · 60 FPS</small></button></div>
      </article>
...
function settings() { return { endpoint: val('endpoint'), streamKey: val('streamKey'), encoder: val('encoder'), rateControl: val('rateControl'), scale: val('scale'), encoderPreset: val('encoderPreset'), width: Number(val('width')), height: Number(val('height')), fps: Number(val('fps')), videoKbps: Number(val('videoKbps')), audioKbps: Number(val('audioKbps')), audioGain: Number(val('audioGain')), keyframeSeconds: Number(val('keyframeSeconds')), bFrames: Number(val('bFrames')), crf: Number(val('crf')), videoSource: val('videoSource'), monitorIndex: Number(val('monitorIndex')), audioSource: val('audioSource') }; }
function link() { const key = val('streamKey').trim(); const a = $('viewerLink'); if (!key) { a.textContent = 'Enter a stream key to create the viewer link'; a.classList.add('empty'); a.href = '#'; return ''; } const url = 'https://b.siobud.com/' + encodeURIComponent(key); a.textContent = url; a.classList.remove('empty'); a.href = '#'; return url; }
...
function updateSourceMode() { const monitor = val('videoSource') === 'Monitor'; if (monitor && val('audioSource') === 'SelectedProcess') $('audioSource').value = 'SystemExceptDiscord'; $('windowSource').hidden = monitor; $('monitorSource').hidden = !monitor; $('audioNote').textContent = val('audioSource') === 'SystemExceptDiscord' ? 'Captures desktop audio while excluding the Discord process tree.' : 'Captures the selected window\'s process tree.'; if (monitor && ![...$('monitorIndex').options].some(o => String(o.value) === String(val('monitorIndex'))) && $('monitorIndex').options.length) $('monitorIndex').selectedIndex = 0; updateWindowMeta(); }
...
function renderSettings(s) { Object.entries(s).forEach(([k, v]) => { const id = k; if ($(id)) set(id, v); }); link(); const chosen = (current.encoders || []).find(o => o.value === val('encoder')); $('encoderNote').textContent = chosen ? chosen.reason : ''; }
function renderState(s) { current = s; renderEncoders(s.encoders || []); renderWindows(); renderMonitors(); renderSettings(s.settings || {}); updateSourceMode(); $('start').disabled = s.running; $('capture').disabled = s.running; $('stop').disabled = !s.running; if (s.status) status(s.status, s.running ? 'live' : 'idle'); }
...
function preset(name) { const s = settings(); if (name === 'budget') Object.assign(s, { encoder:'H264Nvenc', rateControl:'Cbr', scale:'Bilinear', width:1280, height:720, fps:30, videoKbps:2000, audioKbps:192, audioGain:2, encoderPreset:'low-latency', keyframeSeconds:1, bFrames:0 }); if (name === 'motion') Object.assign(s, { encoder:'H264Nvenc', rateControl:'Cbr', scale:'Bilinear', width:1280, height:720, fps:60, videoKbps:3500, audioKbps:192, audioGain:2, encoderPreset:'p1', keyframeSeconds:1, bFrames:0 }); if (name === 'quality') Object.assign(s, { encoder:'H264Nvenc', rateControl:'Cbr', scale:'Lanczos', width:1280, height:720, fps:60, videoKbps:5000, audioKbps:192, audioGain:2, encoderPreset:'p1', keyframeSeconds:1, bFrames:0 }); renderSettings(s); log('Preset applied: ' + name + '.'); }
$('streamKey').addEventListener('input', link); $('window').addEventListener('change', updateWindowMeta); $('monitorIndex').addEventListener('change', updateWindowMeta); $('videoSource').addEventListener('change', updateSourceMode); $('audioSource').addEventListener('change', updateSourceMode);
...
$('copyKey').onclick = () => { const text = val('streamKey').trim(); if (text) { post('copy', { text }); log('Stream key copied.', 'ok'); } }; $('copyLink').onclick = () => { const url = link(); if (url) { post('copy', { text: url }); log('Viewer link copied.', 'ok'); } };
